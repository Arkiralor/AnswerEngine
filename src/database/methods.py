...
class SynchronousMethods:
    ...
    def find(cls, filter_dict: dict = None, collection: str = None, page: int = 1) -> list:
        if not filter_dict:
            results = cls.db[collection].find().skip(
                (page-1)*settings.MAX_ITEMS_PER_PAGE).limit(settings.MAX_ITEMS_PER_PAGE)
        else:
    ...
    def find_and_order(cls, filter_dict:dict=None, collection:str=None, sort_field:str=None, page:int=1) -> list:
        ...
        """
        results = cls.db[collection].find(filter_dict).sort(sort_field, pymongo.DESCENDING).skip((page-1)*settings.MAX_ITEMS_PER_PAGE).limit(settings.MAX_ITEMS_PER_PAGE)
        return list(results)
